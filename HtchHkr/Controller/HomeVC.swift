...
enum AnnotationType {
    case pickup
    case destination
    case driver // unused yet -- maybe popup a notif when driver is close?
}
    ...
    func buttonSelector(forAction action: ButtonAction) {
        ...
        case .requestRide:
            if destinationTxtField.text != "" {
                ...
                showAlert(MSG_INPUT_DESTINATION)
            }
        case .getDirectionsToPassenger:
            DataService.instance.driverIsOnTrip(driverKey: currentUserId, handler: { (isOnTrip, driverKey, tripKey) in
                if isOnTrip == true {
        ...
        DataService.instance.REF_DRIVERS.observeSingleEvent(of: .value, with: { (snapshot) in
            if let driverSnapshot = snapshot.children.allObjects as? [DataSnapshot] {
                for driver in driverSnapshot {
                    if driver.hasChild(ACCOUNT_IS_DRIVER) {
...
extension HomeVC: MKMapViewDelegate {
    ...
    func setCustomRegion(forAnnotationType type: AnnotationType, withCoordinate coordinate: CLLocationCoordinate2D) {
        if type == .pickup {
            let pickupRegion = CLCircularRegion(center: coordinate, radius: 100, identifier: ID_PICKUP)
            manager?.startMonitoring(for: pickupRegion)
        } else if type == .destination {
            let destinationRegion = CLCircularRegion(center: coordinate, radius: 100, identifier: ID_DESTINATION)
